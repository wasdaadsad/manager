                ...
                <div class="widget-box">
                    <div class="widget-header widget-header-flat widget-header-small">
                        <h5 class="widget-title" id="title">
                            <i class="ace-icon fa fa-signal"></i> SDA配置列表
                        </h5>
                            ...
                            <div class="clearfix">
                                <div class="row">
                                    ...
                                    <div class="col-xs-2 nopadding-right">
                                        <input type="text" name="sdaName" class="input-sm col-xs-12"
                                               placeholder="配置名">
                                    </div>

                                    <div class="col-xs-2 nopadding-right">
                                        <input type="text" name="sdaValue" class="input-sm col-xs-12"
                                               placeholder="配置值">
                                    </div>
                                    ...
                                    <div class="col-xs-3">
                                        ...
                                        <button id="addItem" class="btn btn-success btn-xs">
                                            <span class="ace-icon glyphicon glyphicon-ok"></span> 创建新配置
                                        </button>
...
<script type="text/javascript">
    ...
    function execEdit(params) {
        bootbox.confirm("<form id='editForm' ><input type='hidden' name='id' value='"+params.id+"'>配置名:<input type='text' name='sdaName' value='"+params.sdaName+"'><br/>配置值:<input type='text' name='sdaValue' value='"+params.sdaValue+"'/></form>", function (result) {
            var formData = {};
            ...
            if (result) {
                $.ajax({
                    type: 'POST',
                    url:"${contextPath}/admin/SDAConfig/editItem",
                    data:formData,
                    complete:function(){
                        gridReload();
                    }
    ...
    function execDelete(id){
        bootbox.confirm("确定是否删除？", function (result) {
            if (result) {
                $.ajax({
                    type: 'GET',
                    url:"${contextPath}/admin/SDAConfig/deleteItem?id="+id,
                    complete:function(){
                        gridReload();
                    }
                })
    ...
    $("#addItem").on("click", function (e) {
        bootbox.confirm("<form id='addForm' >配置名:<input type='text' name='sdaName'><br/>配置值:<input type='text' name='sdaValue'/></form>", function (result) {
            var formData = {};
            ...
            if (result) {
                $.ajax({
                    type: 'POST',
                    url:"${contextPath}/admin/SDAConfig/addItem",
                    data:formData,
                    complete:function(){
                        gridReload();
                    }
    ...
    jQuery(function ($) {
        ...
        $(grid_selector).jqGrid({
            url: "${contextPath}/admin/SDAConfig/findList.json",
            datatype: "json",
            mtype: "post",
            postData: formData,
            colNames: ['配置名', '配置值', '编辑', '删除'],
            colModel: [
                {name: 'sdaName', index: 'sdaName', align: "center", sortable: false, width: 60},
                {name: 'sdaValue', index: 'sdaValue', align: "center", sortable: false, width: 60},
                {
                    name: 'id',
                    index: 'id',
                    align: "center",
