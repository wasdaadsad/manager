                ...
                <div class="widget-box">
                    <div class="widget-header widget-header-flat widget-header-small">
                        <h5 class="widget-title" id="title">
                            <i class="ace-icon fa fa-signal"></i> 版本列表
                        </h5>
                            ...
                            <div class="clearfix">
                                <div class="row">
                                    ...
                                    <div class="col-xs-2 nopadding-right">
                                        <input type="text" name="versionCode" class="input-sm col-xs-12"
                                               placeholder="版本编号">
                                    </div>
                                ...
                                <div class="clearfix">
                                    <div class="row" style="margin-top: 10px">
                                        <div class="col-xs-10  nopadding-right">
                                            ...
                                            <select name="modelId" id="modelId" class="selectpicker show-tick show-menu-arrow" data-live-search="true" data-width="fit" data-style="btn-success" data-live-search-placeholder="搜索..." >
                                            ...
                                            <select name="osType" id="osType" class="selectpicker show-tick show-menu-arrow" data-live-search="true" data-width="fit" data-style="btn-success" data-live-search-placeholder="搜索..." >
                                                <option value="">全部</option>
...
<script type="text/javascript">
    ...
    function execEdit(cellvalue) {
        layer.open({
            ...
            content: '${contextPath}/uhostmanage/versionManage/editVersion?id=' + cellvalue.id
            + "&versionCode=" + cellvalue.versionCode
            + "&modelId=" + cellvalue.modelId
            + "&osType=" + cellvalue.osType
            + "&packageUrl=" + cellvalue.packageUrl
    ...
    function execDelete(id){
        bootbox.confirm("确定是否删除？", function (result) {
            if (result) {
                $.ajax({
                    type: 'GET',
                    url:"${contextPath}/uhostmanage/versionManage/delete?id="+id,
                    complete:function(){
                        gridReload();
                    }
                })
    ...
    $("#addItem").on("click", function (e) {
        layer.open({
            ...
            content: '${contextPath}/uhostmanage/versionManage/addVersion'
        });
    });
    ...
    jQuery(function ($) {
        ...
        $(grid_selector).jqGrid({
            url: "${contextPath}/uhostmanage/versionManage/list.json",
            datatype: "json",// 从服务器端返回的数据类型
            mtype: "post",// ajax提交方式。POST或者GET，默认GET
            postData: formData,// 此数组内容直接赋值到url上，参数类型：{name1:value1…} 默认空array{}
            colNames: ['版本编号','机型', '系统类型', '升级包URL','创建时间','有效状态','编辑','删除'],//表头
            // 设置表格列的属性
            colModel: [
                {name: 'versionCode', index: 'versionCode', align: "center", sortable: false, width: 40},
                {name: 'modelName', index: 'modelId', align: "center", sortable: false, width: 30},
                {
                    name: 'osType',
                    index: 'osType',
                    align: "center",
                    ...
                    dittype:'select',
                    formatter:'select',
                    editoptions:{value:"6:测试;7:正式;"},
                    stype: "select", sopt:["eq"],searchoptions: {multiple:true, value: ":全部类型;6:测试系统;7:正式系统"}

                },
                {name: 'packageUrl', index: 'packageUrl', align: "center", sortable: false, width: 80},
                {
                    name: 'createTime',
                    index: 'createTime',
                    align: "center",
